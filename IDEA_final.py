...
#        #    open sequence
        ...
        #        Run IUPred
            ...
            iupStatus = os.system(iupcmd)
            #Check run status for IUPred (It might be installed incorrectly)
            if os.path.exists('Iupred_exit_status'):
                os.remove('Iupred_exit_status')
            if iupStatus != 0:
                noIUP = open('Iupred_exit_status','w')
                noIUP.write(str(iupStatus))
                noIUP.close()
                break
            else:
    ...
    def discrete(self):
        if os.path.exists('Iupred_exit_status'):
            print("IUPred was installed incorrectly")
        else:
            def zero(x):
                if x=='X':
                    z='?'
...
def runall(root):
    ...
    result = disorder()
    if os.path.exists('Iupred_exit_status'):
        iupStatus = open('Iupred_exit_status')
        print("IUPred exit status: {}, probably not installed correctly".format(iupStatus.read()))
    else:
